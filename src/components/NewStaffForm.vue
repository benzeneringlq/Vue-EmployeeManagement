...
const headers = [
    ...
    { title: '入职日期', align: 'end', key: 'date' },
    { title: '学历', align: 'end', key: 'cause' }
]
...
async function submit() {
    let result = await globaldata.getNewForm(filter)
...
<template>
    <Maintem>
        ...
        <template v-slot:breadcrumb>
            <v-breadcrumbs class="breadcrumb" :items="['报表管理', '新聘员工报表']"></v-breadcrumbs>
        </template>
        ...
        <template #panel-result-body>
            ...
            <v-btn @click="ConvertToCSV(headers, selectResult, '新聘员工报表.csv')">生成报表</v-btn>
        </template>
    </Maintem>
</template>
<script>
