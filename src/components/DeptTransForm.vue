...
const headers = [
    { title: '编号', align: 'start', key: 'staffID', },
    { title: '原部门名称', align: 'end', key: 'oldID' },
    { title: '新部门名称', align: 'end', key: 'newID' },
...
async function submit() {
    let result = await globaldata.getDepTransForm(filter)
...
<template>
    <Maintem>
        ...
        <template #panel-result-body>
            ...
            <v-btn @click="ConvertToCSV(headers,selectResult,'部门调动报表.csv')">生成报表</v-btn>
        </template>
    </Maintem>
</template>
<script>
